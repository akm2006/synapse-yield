...
export function OnboardingModal({
  ...
  const handleFundAccount = async () => {
    if (!smartAccountAddress || !eoaAddress) {
      addToast({ message: 'Wallet not connected', type: 'error' });
      return;
    }
    const eth = (window as any).ethereum;
    if (!eth) {
    ...
    if (valueWei <= 0n) {
      addToast({ message: 'Invalid amount', type: 'error' });
      return;
    }
    ...
    try {
      ...
      loadingToastId = addToast({ 
        message: `Sending ${amountToFund} MON...`, 
        type: 'loading', 
        duration: 60000 
      });
      const txHash = await eth.request({
        method: 'eth_sendTransaction',
        params: [{ from: eoaAddress, to: smartAccountAddress, value: valueHex }],
      });
      ...
      addToast({ 
        message: `Funding failed: ${err?.message || String(err)}`, 
        type: 'error' 
      });
    } finally {
      setIsFunding(false);
  ...
  const handleCheckBalance = async (notifyParent = false) => {
    ...
    const updatedBalanceWei = parseUnits(balances.native || '0', 18);
    ...
    if (updatedBalanceWei >= MIN_FUNDING_THRESHOLD_WEI) {
      ...
      if (notifyParent && onFundingComplete) onFundingComplete();
    } else {
      addLog('[ONBOARDING] Funding requirement not yet met.');
      addToast({ 
        message: `Account still needs funding (min ${MIN_FUNDING_THRESHOLD_MON} MON).`, 
        type: 'info', 
        duration: 4000 
      });
    }
  ...
  const handleCheckDeployment = async () => {
    ...
    if (deployed) {
      ...
      if (onDeploymentComplete) onDeploymentComplete();
    } else {
      addLog('[ONBOARDING] Account activation not detected yet.');
      addToast({ 
        message: 'Activation transaction not confirmed. Please wait and try again.', 
        type: 'info', 
        duration: 5000 
      });
    }
  ...
  const handleDelegationComplete = () => {
    ...
    addToast({ 
      message: 'Setup complete! You\'re ready.', 
      type: 'success' 
    });
    onDelegationCreated();
    onClose();
  ...
  const handleCopyAddress = () => {
    navigator.clipboard.writeText(smartAccountAddress)
      .then(() => addToast({ 
        message: 'Smart Account address copied!', 
        type: 'success', 
        duration: 3000 
      }))
      .catch(() => addToast({ 
        ...
        return (
          <div className="space-y-6 text-center">
            ...
            <div className="space-y-3 pt-4">
              <label htmlFor="fundAmount" className="block text-sm font-medium text-gray-300 text-left">
                Amount to Fund (MON)
              </label>
              <input
                id="fundAmount"
        ...
        return (
          <div className="space-y-6 text-center">
            ...
            <div className="bg-yellow-900/20 border border-yellow-700/50 p-3 rounded-lg text-xs text-yellow-300 flex items-start gap-2">
              ...
              <span>
                Go to the <Link href="/swap" className="font-semibold underline hover:text-yellow-200">Swap Page</Link>, select the 'Transfer' tab, and send a tiny amount (e.g., 0.001 MON) from your Smart Account to any address.
              </span>
            </div>
            <motion.button
                  ...
                  {steps.map((step, index) => (
                    <Fragment key={step.id}>
                        ...
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-colors duration-300 ${
                          ...
                          {currentStep > step.id ? (
                            ...
                            <step.icon className={`h-4 w-4 ${
                              currentStep === step.id ? 'text-blue-300' : 'text-gray-500'
                            }`} />
                          )}
                        </div>
                      ...
                      {index < steps.length - 1 && (
                        <div className={`flex-grow h-0.5 mt-4 transition-colors duration-300 ${
